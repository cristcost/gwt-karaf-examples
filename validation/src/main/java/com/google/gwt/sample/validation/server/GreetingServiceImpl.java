...
public class GreetingServiceImpl extends RemoteServiceServlet implements GreetingService {
  ...
  /**
   * In Osgi Validation.buildDefaultValidatorFactory().getValidator(); does not
   * work,
   * 
   * see http://docs.jboss.org/hibernate/validator/4.0.1/reference/en/html/
   * validator-bootstrapping.html#d0e2185
   * 
   * using customized bootstrap
   */
  private final Validator validator = OsgiValidation.getValidator();

  public SafeHtml greetServer(Person person) throws IllegalArgumentException,
    ConstraintViolationException {
    // Verify that the input is valid.
    Set<ConstraintViolation<Person>> violations =
        validator.validate(person, Default.class, ServerGroup.class);
